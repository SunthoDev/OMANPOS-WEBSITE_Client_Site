...
const AdminSeeAllUser = () => {
    ...
    const { data: adminAllUsers = [], refetch } = useQuery({
        queryKey: ["users"],
        queryFn: async () => {
            const res = await fetch("https://server.docswellet.com/users");
            return res.json();
        },
    });
    ...
    let HandleAdmin = (id) => {
        ...
        fetch(`https://server.docswellet.com/AdminUpdateRoleAdmin/${id}`, {
            method: "PATCH",
        })
            .then(res => res.json())
            .then(data => {
    ...
    let handleUser = (id) => {
        ...
        fetch(`https://server.docswellet.com/AdminUpdateRoleUser/${id}`, {
            method: "PATCH",
        })
            .then(res => res.json())
            .then(data => {
    ...
    let HandleDelete = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                fetch(`https://server.docswellet.com/AdminDeleteUsers/${id}`, {
                    method: "DELETE",
                })
                    .then(res => res.json())
                    .then(data => {
                    ...
                    <table className="table">
                        <thead>
                            <tr>
                                ...
                                <th className="text-[14px] font-[600] text-white">Password</th>
                        ...
                        <tbody>
                            ...
                            {
                                adminAllUsers?.slice().reverse().map(allUser => <UserAllData HandleAdmin={HandleAdmin} handleUser={handleUser} HandleDelete={HandleDelete} key={allUser._id} allUser={allUser}></UserAllData>)
                            }
